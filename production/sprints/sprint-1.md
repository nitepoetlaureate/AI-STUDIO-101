...
| **S1-05** | System 3: Audio Manager | `godot-gdscript-specialist` | 1 | S1-01, S1-03 | 4 buses (Master, Music, SFX, Ambient); `play_sfx()`, `play_music()`, `crossfade_music()`, `set_bus_volume()` API; no direct `AudioStreamPlayer` calls from gameplay; GUT: all API callable headless without error | **Done** (Session 013 — buses + API + `test_audio_manager.gd`; catalog WAV/OGG under `res://assets/audio/` optional) |
