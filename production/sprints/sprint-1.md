...
|-------|------|:--------:|----------------|
| 1 | `BonnieController` | 10 | Traversal; **`LevelManager` LOS pass** syncs `visible_to_bonnie` (A+C); Bonnie notifies + LOS rig API only |
...
├── bonnie_camera_config.tres      # S1-10 — camera look-ahead / framing
├── npc/
...
|----|------|-------------|:---:|---|---|---|
| **S1-01** | Infrastructure: `src/` scaffold, GUT 7.x install, `project.godot` baseline | `godot-specialist` | 1 | — | `src/` structure matches spec; GUT installed and first run exits clean; autoloads registered; `gdcli lint src/` exits 0 | **Done** (Session 010) |
| **S1-02** | ADR-001: Production Architecture | `lead-programmer` | 1 | — | `docs/architecture/ADR-001-production-architecture.md` committed; covers autoload rationale, dependency direction, NpcState pattern, Systems 8 & 15 stub contracts, frame execution order | **Done** (Session 010) |
...
| **S1-08** | Config `.tres` data files — all 11 MVP systems | `godot-gdscript-specialist` | 1 | S1-06 | All typed `.tres` under `assets/data/` with GDD defaults; **tuning** literals live in `.tres` (see **ADR-001** — algorithmic sentinels allowed in `.gd`); `gdcli` validates resources load clean | **Done** (`input_system_config`, `level_config`, `bonnie_traversal_config`, `line_of_sight_config`, **`bonnie_camera_config`**, `chaos_meter_config`, **`chaos_meter_ui_config`**, `social_system_config`, `npc/michael_profile`, `npc/christen_profile`; audit **2026-04-23** + camera **2026-04-23**) |
| **S1-09** | System 6: BONNIE Traversal — production rewrite | `godot-gdscript-specialist` | 3 | S1-04, S1-08 | All 13 states; all values from `.tres`; signals `state_changed`, `stimulus_radius_updated`; **`visible_to_bonnie` = distance + LOS** (`SESSION-015-PROMPT.md`); `VisibilityLedger` + LevelManager LOS pass; AC-T01 through AC-T08 all pass; prototype moved to `prototypes/archived/` only after AC validation | **In progress** — engineering: `test_bonnie_controller_production.gd` + LOS; `BonnieController` applies impulses to `InteractiveObject` via `receive_impact`; **human** AC-T01–T08 + archive: **PLAYTEST-004** only |
| **S1-10** | System 4: Camera System | `godot-gdscript-specialist` | 2 | S1-09 | Look-ahead per state via `LOOK_AHEAD_BY_STATE` table; smooth follow (no whip on reversal); ground-biased vertical framing; all AC-T08 pass | **In progress** — engineering: `test_bonnie_camera_production.gd` (incl. `test_lookahead_running_exceeds_walking_on_mock_bonnie`); **human** AC-T08: **PLAYTEST-004** |
| **S1-11** | System 9: Reactive NPC System | `godot-gdscript-specialist` | 3 | S1-06, S1-07, S1-09 | 11-state machine; `emotional_level` decay (§4.1); `comfort_receptivity` transitions (§4.3); Domino Rally cascade depth ≤ 2 with `cascade_source_id` loop prevention; Michael + Christen from `.tres`; phase routine timer (pauses outside ROUTINE); Christen arrival trigger on Michael Afternoon → Evening; AC-01 through AC-11 pass; GUT: decay formula, cascade loop blocked, FED gate both-conditions required | **In progress** — `NpcController.gd` + `class_name`; decay §4.1 + REACTING/RECOVERING/VULNERABLE/FED + cascade to Christen + overwhelm thresholds on profile; GUT `test_npc_controller_decay.gd`; **routine phases / Christen evening trigger** = timer arrival MVP (120s) not full GDD phase graph — **producer exception** footnote §S1-PROD-001; **human** AC-01–11: **PLAYTEST-004** |
| **S1-12** | System 12: Bidirectional Social System | `godot-gdscript-specialist` | 2 | S1-11 | All 5 interactions (Proximity, Rub, Lap Sit, Purr, Meow); NPC state gate + BONNIE movement state gate enforced; levity multiplier with timestamp; RECOVERING always levity-eligible; `recovering_comfort_stacks` per NPC; `passive_accumulator` levity-window protection; AWARE conversion via `deescalation_event` signal; AC-S01 through AC-S13 pass; GUT: levity truth table, passive equilibrium math, meow resolution | **In progress** — `SocialSystem.gd` proximity + `interact` charm; levity via `last_chaos_timestamp`; GUT `test_social_levity.gd`; **not** full interaction matrix / AWARE signal — **producer exception** §S1-PROD-002; **human** AC-S01–S13: **PLAYTEST-004** |
| **S1-13** | System 7: Interactive Objects — physics foundation | `godot-gdscript-specialist` | 2 | S1-09, S1-05 | `InteractiveObject.gd` on RigidBody2D with explicit CharacterBody2D→RigidBody2D impulse application (Godot 4 requires this); weight classes (Light/Medium/Heavy/Glass/Liquid) from GDD §3.1; emits `chaos_event_received(npc_id, severity)` to `SocialSystem` and `object_chaos_event(value)` to `ChaosEventBus`; two objects in test scene (CoffeeMug Light, Bookshelf Medium); physics material tuning for weight feel; acceptance: slide into CoffeeMug at full speed sends it off table, decrements Michael's goodwill by `0.10`, increments `chaos_fill` by `0.02` within 1 frame; object feels like it has real mass | **In progress** — `InteractiveObject.gd` + `BonnieController` slide/run impulse bridge + `test_interactive_object.gd` + 2 instances in `test_apartment`; goodwill hit via `NpcController.apply_chaos_goodwill_hit`; sprint numeric AC needs **human** slide playtest |
| **S1-14** | Stub interfaces: Systems 8 & 15 | `godot-gdscript-specialist` | 1 | S1-01 | `ChaosEventBus` defines `object_chaos_event(value: float)` and `pest_caught(pest_type: int)`; `ChaosMeter` subscribes; stubs silent in Sprint 1; GUT: manual signal emit correctly increments `chaos_fill` | **Done** (2026-04-23 — evidence: `tests/unit/test_chaos_event_bus_meter.gd` `test_object_chaos_event_increments_chaos_fill` + `test_pest_caught_increments_chaos_fill`; `ChaosMeter` subscribes to both signals; **S1-15** full formulas separate) |
| **S1-15** | System 13: Chaos Meter | `godot-gdscript-specialist` | 2 | S1-11, S1-12, S1-14 | Accumulates `chaos_fill` from REACTING events (§4.1 formula); derives `social_fill` each frame (§4.4 formula, normalized by `active_npc_count`); `chaos_event_count` session counter; `meter_value` + `meter_state` computed; overwhelm check (Michael 8, Christen 7); `FeedingPathType` tracked; invariant asserted in `_ready()`; AC-CM-01 through AC-CM-09 pass; GUT: pure chaos plateau, pure charm gate, combined path, hunger boost | **In progress** — `ChaosMeter.gd` REACTING contribution + social_fill §4.4 + `chaos_event_count` + overwhelm via profile; GUT `test_chaos_meter_formulas.gd` + `test_npc_state.gd` invariant; **AC-CM-10** (pest cannot satisfy FED): **producer exception** §S1-PROD-003 — pest stub value only until pest gameplay slice |
| **S1-16** | System 23: Chaos Meter UI | `ui-programmer` | 2 | S1-15, S1-05, S1-03 | Bowl widget bottom-right; chaos zone 16×35px, social zone 16×29px, cap line always visible; fill chase interpolation at `fill_chase_speed = 4.0` with integer pixel quantization; all 6 state visual changes; overwhelm anti-signal (stays HOT); audio signals on state transitions; reduced-motion mode; programmer-art placeholders (ColorRect layers); AC-UI-01 through AC-UI-13 pass | **In progress** — `ChaosMeterUI.gd` ColorRect bowl + fill chase from `ChaosMeterUIConfig` / `ChaosMeterConfig`; **not** full AC-UI (audio, reduced-motion, 6-state art) — **human** AC-UI: **PLAYTEST-004** |
| **S1-17** | Test Apartment Level Assembly | `godot-specialist` | 2 | S1-09–S1-16 | `test_apartment.tscn` with 3 rooms (kitchen, living room, bedroom) as TileMap-based floors with surface type metadata (hardwood=kitchen, carpet=living room+bedroom); room boundary Area2D triggers; all signal connections at scene level; `LevelManager` loads correctly; Christen deactivated until 120s arrival timer; Michael starts in kitchen; 2+ interactive objects placed on surfaces; `gdcli run --headless` completes without errors | **In progress** — `scenes/production/test_apartment.tscn` + `test_apartment_root.gd`; 3 `register_room` rects; Christen `PROCESS_MODE_DISABLED` 120s; 2× `InteractiveObject`; headless `godot … --quit-after 8` clean; **producer exception** §S1-PROD-004: TileMap + surface metadata + Area2D room triggers deferred (StaticBody2D floor MVP) |
| **S1-18** | Core Loop Playtest Validation | `qa-tester` | 1 | S1-17 | Undocumented playtester can: move BONNIE through all key states, trigger Michael REACTING ×2, charm during RECOVERING, watch meter fill on both axes, see all 6 UI states, confirm FED fires, confirm overwhelm path withholds FEEDING visual; playtest report → `prototypes/bonnie-traversal/PLAYTEST-004.md` | **In progress** — template + preconditions in **PLAYTEST-004**; awaiting blind run |

**Must Have total: ~35 agent-sessions** (adjusted: S1-13 1→2, S1-17 1→2)

### Producer exceptions (Mycelium + sprint — no silent deferrals)

| ID | Scope | Record |
|----|--------|--------|
| **§S1-PROD-001** | S1-11 routine / Christen evening | Christen uses **120s** `LevelManager.level_elapsed_time` arrival MVP; full Michael phase graph (morning/afternoon/evening) not authored in scene. |
| **§S1-PROD-002** | S1-12 interactions | Proximity tick + single `interact` charm; not all five discrete interaction types / movement gates / `deescalation_event`. |
| **§S1-PROD-003** | S1-15 AC-CM-10 | Pest → `chaos_fill` stub only; FED gate not wired to pest path per GDD AC-CM-10 until pest slice. |
| **§S1-PROD-004** | S1-17 tiles | Rooms = `Rect2` registration + static floor; TileMap surface metadata + boundary `Area2D` triggers deferred. |
| **§S1-PROD-005** | DoD S1-23 | Debug overlay not required for **PLAYTEST-004** template; enable in follow-up or add overlay before final Gate 3 if QA requires live values. |
...
| S1-20 | GUT tests: NPC System formulas | `godot-gdscript-specialist` | 1 | S1-11 | **Partial** — `test_npc_controller_decay.gd` (decay); expand for cascade/FED |
| S1-21 | GUT tests: Social System | `godot-gdscript-specialist` | 1 | S1-12 | **Partial** — `test_social_levity.gd`; expand truth table |
| S1-22 | GUT tests: Chaos Meter formulas | `godot-gdscript-specialist` | 1 | S1-15 | **Partial** — `test_chaos_meter_formulas.gd`; expand charm-only / hunger |
...
| `gdcli` (`npx -y gdcli-godot`) | **CI:** GUT + `doctor` + **`script lint`** on `push`/`PR` (`.github/workflows/godot-ci.yml`) | Lint noise on legacy dirs | Scope lint to `src/` if needed |
